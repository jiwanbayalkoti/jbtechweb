        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($request->invoice)
                        <strong>{{ $request->invoice->invoice_number }}</strong>
                    ...
                    <td>
                        <a href="{{ route('tenant.plan-requests.show', $request) }}" class="btn btn-sm btn-info mb-1">
                            <i class="fas fa-eye"></i> View
                        </a>
                        @if($request->status !== 'approved')
                        <form method="POST" action="{{ route('tenant.plan-requests.approve', $request) }}" class="mb-1">
                            @csrf
                        ...
                        @endif
                        @if($request->invoice)
                        <form method="POST" action="{{ route('tenant.plan-requests.send-email', $request) }}" class="mb-1">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('tenant.plan-requests.mark-read', $request) }}">
                            @csrf
                ...
                @empty
                <tr><td colspan="6" class="text-center">No plan requests</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
